...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedMenu) {
      try {
        const parsed = JSON.parse(savedMenu) as DailyMenuType & { date?: string }
    ...
    <div className="app">
      <header className="app-header">
        ...
        <p className="diet-source">
          {userDiet.dietData ? 'Dieta personalizzata' : userDiet.dailyMenus.length !== dailyMenus.length ? 'Dieta personalizzata' : 'Dieta predefinita'} ({userDiet.dailyMenus.length} menu)
        </p>
        <button
          type="button"
      ...
      <main className="app-main">
        {currentMenu && (
          <DailyMenu
            ...
            dietData={userDiet.dietData ?? buildDietDataFromMenus(userDiet.dailyMenus) ?? defaultDietData}
          />
        )}
      </main>
      ...
      <footer className="app-footer">
        <p>
          ...
          <HeartIcon
            size={14}
            style={{ margin: '0 0.25rem', color: '#e74c3c', verticalAlign: 'middle', display: 'inline-block' }}
          />{' '}
          - Menu Dietetici PWA
        </p>
      </footer>
