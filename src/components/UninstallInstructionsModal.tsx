...
function getSteps(platform: UninstallPlatform, isStandalone: boolean): Step[] {
  switch (platform) {
    case 'ios':
      return [
        { icon: <IconHandFinger size={STEP_SIZE} />, text: 'Tieni premuta l’icona dell’app sulla schermata Home' },
        { icon: <IconTrash size={STEP_SIZE} />, text: 'Tocca «Rimuovi App»' },
        { icon: <IconCircleCheck size={STEP_SIZE} />, text: 'Conferma con «Rimuovi»' },
      ]
    case 'android':
      return [
        { icon: <IconHandFinger size={STEP_SIZE} />, text: 'Tieni premuta l’icona dell’app' },
        { icon: <IconTrash size={STEP_SIZE} />, text: 'Tocca «Disinstalla»' },
        { icon: <IconSettings size={STEP_SIZE} />, text: 'Oppure: Impostazioni → App → cerca l’app → Disinstalla' },
      ]
    case 'chrome':
      return [
        {
        ...
        { icon: <IconPointer size={STEP_SIZE} />, text: 'Tasto destro sull’app' },
        { icon: <IconTrash size={STEP_SIZE} />, text: 'Clicca «Rimuovi da Chrome»' },
      ]
    case 'edge':
      return [
        {
        ...
        { icon: <IconDots size={STEP_SIZE} />, text: 'Clicca ⋮ sull’icona dell’app' },
...
export default function UninstallInstructionsModal({ platform, onClose, isStandalone = false }: UninstallInstructionsModalProps) {
  const steps = getSteps(platform, isStandalone)
  const title = platform === 'other' ? 'Come disinstallare l’app' : 'Come disinstallare l’app'
  const isChromeOrEdge = platform === 'chrome' || platform === 'edge'
  const showVaiAlleAppButton = isChromeOrEdge && !isStandalone
  const openAppsUrl = platform === 'chrome' ? 'chrome://apps' : platform === 'edge' ? 'edge://apps' : null
  ...
  return (
    <Modal
      ...
      primaryLabel={isStandalone ? 'Ho capito' : showVaiAlleAppButton ? 'Vai alle app' : undefined}
      onPrimaryClick={
        isStandalone
          ? onClose
    ...
    >
      {platform === 'other' ? (
        <p className="uninstall-modal-fallback">
          Se l’hai aggiunta da Chrome o Edge: menu del browser → Estensioni e app installate / Gestisci app → rimuovi l’app.
          Su Mac dalla Home: trascina l’icona fuori dal Dock o da Launchpad.
        </p>
      ) : (
        <>
          ...
          {!isStandalone && platform === 'chrome' && (
            <p className="uninstall-modal-fallback-note">
              Se il pulsante non apre la pagina: apri Chrome, digita <strong>chrome://apps</strong> nella barra degli indirizzi e premi Invio.
            </p>
          )}
          {!isStandalone && platform === 'edge' && (
            <p className="uninstall-modal-fallback-note">
              Se il pulsante non apre la pagina: apri Edge, digita <strong>edge://apps</strong> nella barra degli indirizzi e premi Invio.
            </p>
          )}
        </>
      )}
